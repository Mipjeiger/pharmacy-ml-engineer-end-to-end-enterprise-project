"""Bronze layer pipeline for pharmacy data."""
...
def run_bronze_pipeline(batch_limit=1000):
    ...
    # Ensure fethching can handle large datasets by limiting the number of records processed
    for record in fetch_pharmacy_sales():
        # Send to Kafka
        producer.send(TOPIC_BRONZE, value=record)
        write_json("bronze", record)
    ...
    producer.flush()
    logger.info(f"Bronze pipeline completed. Total records processed: {count}.")
